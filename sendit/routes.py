...
from sendit import db
from sendit.models import ist_time  # if you defined this helper
...
def folders():
    ...
    if form.validate_on_submit():
        # ✅ Create folder with user_id automatically set
        ...
        new_folder = Folder(
            ...
            created_at=ist_time(),
            parent_id = parent_id,
            user_id=current_user.id  # ✅ important fix here
        )
    ...
    # Only fetch folders belonging to the logged-in user
...
def upload():
    form = UploadForm()
    form.folder_id.choices =  [(f.id,f.name) for f in Folder.query.all()]
    ...
    if form.validate_on_submit():
        ...
        upload_record = FileUpload(filename=filename, description=description, folder_id = folder_id , user_id = current_user.id)
...
def uploads_by_folder(folder_id):
    ...
    return render_template('uploads.html', folder=folder , files= files , subfolders=subfolders , breadcrumbs=breadcrumbs)
...
def update_file(id):
    ...
    return render_template('uploads_by_folder.html', form=form, file=file)
...
def rename_file(id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('uploads_by_folder', form=form , file=file))
    elif request.method == 'GET':
...
def save_picture(form_picture):
    ...
    # Resize image (optional but nice)
    # output_size = (125, 125)
